...
import { C as isTimeInput, D as unrefTimer, F as DEFAULT_ATTRIBUTE_VALUE_LENGTH_LIMIT, H as isAttributeValue, J as suppressTracing, L as TracesSamplerValues, M as getEnv, N as getEnvWithoutDefaults, T as millisToHrTime, U as sanitizeAttributes, W as W3CBaggagePropagator, b as hrTime, g as ExportResultCode, j as otperformance, m as CompositePropagator, n as init_esm, o as merge, q as isTracingSuppressed, r as BindOnceFuture, u as W3CTraceContextPropagator, v as addHrTimes, w as isTimeInputHrTime, x as hrTimeDuration, y as getTimeOrigin, z as globalErrorHandler } from "./core+[...].mjs";
import { t as Resource } from "./resources+[...].mjs";
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/internal/utils.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/trace/SemanticAttributes.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/trace/index.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/resource/SemanticResourceAttributes.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/resource/index.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/stable_attributes.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/stable_metrics.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/node_modules/@opentelemetry/semantic-conventions/build/src/index.js
...
var Span = function() {
	...
	* */
	function Span(parentTracer, context, spanName, spanContext, kind, parentSpanId, links, startTime, _deprecatedClock, attributes) {
		...
		this.status = { code: import_src.SpanStatusCode.UNSET };
		this.endTime = [0, 0];
		this._ended = false;
		this._duration = [-1, -1];
		this.name = spanName;
	...
	*/
	...
	Span.prototype.setStatus = function(status) {
		...
		if (this.status.message != null && typeof status.message !== "string") {
			import_src.diag.warn("Dropping invalid status.message of type '" + typeof status.message + "', expected 'string'");
	...
	Span.prototype.recordException = function(exception, time) {
		var attributes = {};
		if (typeof exception === "string") attributes[import_src$1.SEMATTRS_EXCEPTION_MESSAGE] = exception;
		else if (exception) {
			if (exception.code) attributes[import_src$1.SEMATTRS_EXCEPTION_TYPE] = exception.code.toString();
			else if (exception.name) attributes[import_src$1.SEMATTRS_EXCEPTION_TYPE] = exception.name;
			if (exception.message) attributes[import_src$1.SEMATTRS_EXCEPTION_MESSAGE] = exception.message;
			if (exception.stack) attributes[import_src$1.SEMATTRS_EXCEPTION_STACKTRACE] = exception.stack;
		}
		if (attributes[import_src$1.SEMATTRS_EXCEPTION_TYPE] || attributes[import_src$1.SEMATTRS_EXCEPTION_MESSAGE]) this.addEvent(ExceptionEventName, attributes, time);
	...
	Span.prototype._isSpanEnded = function() {
		if (this._ended) import_src.diag.warn("Can not execute the operation on ended Span {traceId: " + this._spanContext.traceId + ", spanId: " + this._spanContext.spanId + "}");
		return this._ended;
	};
...
var ParentBasedSampler = function() {
	...
	ParentBasedSampler.prototype.shouldSample = function(context, traceId, spanName, spanKind, attributes, links) {
		var parentContext = import_src.trace.getSpanContext(context);
...
function buildSamplerFromEnv(environment) {
	...
	switch (environment.OTEL_TRACES_SAMPLER) {
		...
		case TracesSamplerValues.TraceIdRatio: return new TraceIdRatioBasedSampler(getSamplerProbabilityFromEnv(environment));
		case TracesSamplerValues.ParentBasedTraceIdRatio: return new ParentBasedSampler({ root: new TraceIdRatioBasedSampler(getSamplerProbabilityFromEnv(environment)) });
		default:
			import_src.diag.error("OTEL_TRACES_SAMPLER value \"" + environment.OTEL_TRACES_SAMPLER + " invalid, defaulting to " + FALLBACK_OTEL_TRACES_SAMPLER + "\".");
...
function getSamplerProbabilityFromEnv(environment) {
	...
	if (isNaN(probability)) {
		import_src.diag.error("OTEL_TRACES_SAMPLER_ARG=" + environment.OTEL_TRACES_SAMPLER_ARG + " was given, but it is invalid, defaulting to " + DEFAULT_RATIO + ".");
		return DEFAULT_RATIO;
	}
	if (probability < 0 || probability > 1) {
		import_src.diag.error("OTEL_TRACES_SAMPLER_ARG=" + environment.OTEL_TRACES_SAMPLER_ARG + " was given, but it is out of range ([0..1]), defaulting to " + DEFAULT_RATIO + ".");
		return DEFAULT_RATIO;
	}
	return probability;
}
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/build/esm/utility.js
init_esm();
/**
...
function mergeConfig(userConfig) {
	var perInstanceDefaults = { sampler: buildSamplerFromEnv() };
...
var BatchSpanProcessorBase = function() {
	function BatchSpanProcessorBase(_exporter, config) {
		...
		if (this._maxExportBatchSize > this._maxQueueSize) {
			import_src.diag.warn("BatchSpanProcessor: maxExportBatchSize must be smaller or equal to maxQueueSize, setting maxExportBatchSize to match maxQueueSize");
	...
	BatchSpanProcessorBase.prototype.onEnd = function(span) {
		...
		if ((span.spanContext().traceFlags & import_src.TraceFlags.SAMPLED) === 0) return;
	...
	BatchSpanProcessorBase.prototype._addToBuffer = function(span) {
		if (this._finishedSpans.length >= this._maxQueueSize) {
			if (this._droppedSpansCount === 0) import_src.diag.debug("maxQueueSize reached, dropping spans");
			this._droppedSpansCount++;
			return;
		}
		if (this._droppedSpansCount > 0) {
			import_src.diag.warn("Dropped " + this._droppedSpansCount + " spans because maxQueueSize reached");
			this._droppedSpansCount = 0;
		}
			...
			import_src.context.with(suppressTracing(import_src.context.active()), function() {
				...
				var doExport = function() {
					return _this._exporter.export(spans, function(result) {
						...
						else reject((_a = result.error) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error("BatchSpanProcessor: span export failed"));
					});
				};
				...
				for (var i = 0, len = spans.length; i < len; i++) {
					...
					if (span.resource.asyncAttributesPending && span.resource.waitForAsyncAttributes) {
						pendingResources !== null && pendingResources !== void 0 || (pendingResources = []);
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/build/esm/platform/node/export/BatchSpanProcessor.js
...
//#endregion
//#region node_modules/@opentelemetry/sdk-trace-base/build/esm/platform/node/RandomIdGenerator.js
var SPAN_ID_BYTES = 8;
...
var Tracer = function() {
	...
	*/
	Tracer.prototype.startSpan = function(name, options, context) {
		...
		if (context === void 0) context = import_src.context.active();
		if (options.root) context = import_src.trace.deleteSpan(context);
		var parentSpan = import_src.trace.getSpan(context);
		if (isTracingSuppressed(context)) {
			import_src.diag.debug("Instrumentation suppressed, returning Noop Span");
			var nonRecordingSpan = import_src.trace.wrapSpanContext(import_src.INVALID_SPAN_CONTEXT);
		...
		var spanKind = (_a = options.kind) !== null && _a !== void 0 ? _a : import_src.SpanKind.INTERNAL;
		...
		var traceFlags = samplingResult.decision === import_src.SamplingDecision.RECORD_AND_SAMPLED ? import_src.TraceFlags.SAMPLED : import_src.TraceFlags.NONE;
		var spanContext = {
			traceId,
			spanId,
			traceFlags,
		...
		if (samplingResult.decision === import_src.SamplingDecision.NOT_RECORD) {
			...
			var nonRecordingSpan = import_src.trace.wrapSpanContext(spanContext);
			return nonRecordingSpan;
		}
		var initAttributes = sanitizeAttributes(Object.assign(attributes, samplingResult.attributes));
		return new Span(this, context, name, spanContext, spanKind, parentSpanId, links, options.startTime, void 0, initAttributes);
	};
	Tracer.prototype.startActiveSpan = function(name, arg2, arg3, arg4) {
		...
		var parentContext = ctx !== null && ctx !== void 0 ? ctx : import_src.context.active();
		var span = this.startSpan(name, opts, parentContext);
		var contextWithSpanSet = import_src.trace.setSpan(parentContext, span);
...
var MultiSpanProcessor = function() {
	...
	MultiSpanProcessor.prototype.onStart = function(span, context) {
		var e_2, _a;
		try {
			for (var _b = __values(this._spanProcessors), _c = _b.next(); !_c.done; _c = _b.next()) _c.value.onStart(span, context);
		} catch (e_2_1) {
			e_2 = { error: e_2_1 };
		} finally {
			try {
	...
	MultiSpanProcessor.prototype.onEnd = function(span) {
		var e_3, _a;
		try {
			for (var _b = __values(this._spanProcessors), _c = _b.next(); !_c.done; _c = _b.next()) _c.value.onEnd(span);
		} catch (e_3_1) {
			e_3 = { error: e_3_1 };
		} finally {
			try {
...
var BasicTracerProvider = function() {
	function BasicTracerProvider(config) {
		...
		if (mergedConfig.mergeResourceWithDefaults) this.resource = Resource.default().merge(this.resource);
		this._config = Object.assign({}, mergedConfig, { resource: this.resource });
	...
	*/
	BasicTracerProvider.prototype.addSpanProcessor = function(spanProcessor) {
		if (this._registeredSpanProcessors.length === 0) this.activeSpanProcessor.shutdown().catch(function(err) {
			return import_src.diag.error("Error while trying to shutdown current span processor", err);
		});
	...
	*/
	BasicTracerProvider.prototype.register = function(config) {
		if (config === void 0) config = {};
		import_src.trace.setGlobalTracerProvider(this);
		if (config.propagator === void 0) config.propagator = this._buildPropagatorFromEnv();
		if (config.contextManager) import_src.context.setGlobalContextManager(config.contextManager);
		if (config.propagator) import_src.propagation.setGlobalPropagator(config.propagator);
	};
	...
	*/
	...
	BasicTracerProvider.prototype._buildPropagatorFromEnv = function() {
		...
		var validPropagators = uniquePropagatorNames.map(function(name) {
			var propagator = _this._getPropagator(name);
			if (!propagator) import_src.diag.warn("Propagator \"" + name + "\" requested through environment variable is unavailable.");
			return propagator;
		...
		else return new CompositePropagator({ propagators: validPropagators });
	};
	BasicTracerProvider.prototype._buildExporterFromEnv = function() {
		var exporterName = getEnv().OTEL_TRACES_EXPORTER;
		if (exporterName === "none" || exporterName === "") return;
		var exporter = this._getSpanExporter(exporterName);
		if (!exporter) import_src.diag.error("Exporter \"" + exporterName + "\" requested through environment variable is unavailable.");
		return exporter;
	};
	BasicTracerProvider._registeredPropagators = /* @__PURE__ */ new Map([["tracecontext", function() {
...
//#endregion
export { SamplingDecision as n, BasicTracerProvider as t };
