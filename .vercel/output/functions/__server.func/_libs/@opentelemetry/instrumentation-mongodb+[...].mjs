...
//#region node_modules/@opentelemetry/instrumentation-mongodb/build/src/internal-types.js
var require_internal_types = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	})(exports.MongodbCommandType || (exports.MongodbCommandType = {}));
}));
//#endregion
//#region node_modules/@opentelemetry/instrumentation-mongodb/build/src/version.js
...
//#endregion
//#region node_modules/@opentelemetry/instrumentation-mongodb/build/src/instrumentation.js
var require_instrumentation = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	exports.MongoDBInstrumentation = class MongoDBInstrumentation extends instrumentation_1.InstrumentationBase {
		constructor(config = {}) {
		...
		init() {
			...
			return [new instrumentation_1.InstrumentationNodeModuleDefinition("mongodb", [">=3.3.0 <4"], void 0, void 0, [new instrumentation_1.InstrumentationNodeModuleFile("mongodb/lib/core/wireprotocol/index.js", [">=3.3.0 <4"], v3PatchConnection, v3UnpatchConnection)]), new instrumentation_1.InstrumentationNodeModuleDefinition("mongodb", [">=4.0.0 <7"], void 0, void 0, [
		...
		_getV3ConnectionPatches() {
			return {
				v3PatchConnection: (moduleExports) => {
					if ((0, instrumentation_1.isWrapped)(moduleExports.insert)) this._unwrap(moduleExports, "insert");
					this._wrap(moduleExports, "insert", this._getV3PatchOperation("insert"));
					if ((0, instrumentation_1.isWrapped)(moduleExports.remove)) this._unwrap(moduleExports, "remove");
					this._wrap(moduleExports, "remove", this._getV3PatchOperation("remove"));
					if ((0, instrumentation_1.isWrapped)(moduleExports.update)) this._unwrap(moduleExports, "update");
					this._wrap(moduleExports, "update", this._getV3PatchOperation("update"));
					if ((0, instrumentation_1.isWrapped)(moduleExports.command)) this._unwrap(moduleExports, "command");
					this._wrap(moduleExports, "command", this._getV3PatchCommand());
					if ((0, instrumentation_1.isWrapped)(moduleExports.query)) this._unwrap(moduleExports, "query");
					this._wrap(moduleExports, "query", this._getV3PatchFind());
					if ((0, instrumentation_1.isWrapped)(moduleExports.getMore)) this._unwrap(moduleExports, "getMore");
		...
		_getV4SessionsPatches() {
			return {
				v4PatchSessions: (moduleExports) => {
					if ((0, instrumentation_1.isWrapped)(moduleExports.acquire)) this._unwrap(moduleExports, "acquire");
					this._wrap(moduleExports.ServerSessionPool.prototype, "acquire", this._getV4AcquireCommand());
					if ((0, instrumentation_1.isWrapped)(moduleExports.release)) this._unwrap(moduleExports, "release");
		...
		_getV4AcquireCommand() {
			...
			return (original) => {
				return function patchAcquire() {
					...
					if (nSessionsBeforeAcquire === nSessionsAfterAcquire) instrumentation._connectionsUsage.add(1, {
						state: "used",
		...
		_getV4ConnectionPoolPatches() {
			return {
				v4PatchConnectionPool: (moduleExports) => {
					...
					if ((0, instrumentation_1.isWrapped)(poolPrototype.checkOut)) this._unwrap(poolPrototype, "checkOut");
			...
			return (original) => {
				return function patchedConnect(options, callback) {
					if (original.length === 1) {
						...
						if (result && typeof result.then === "function") result.then(() => instrumentation.setPoolName(options), () => void 0);
						return result;
					}
		...
		_getV4ConnectionPatches() {
			return {
				v4PatchConnectionCallback: (moduleExports) => {
					if ((0, instrumentation_1.isWrapped)(moduleExports.Connection.prototype.command)) this._unwrap(moduleExports.Connection.prototype, "command");
		...
		_getV3PatchOperation(operationName) {
			...
			return (original) => {
				return function patchedServerCommand(server, ns, ops, options, callback) {
					...
					if (skipInstrumentation || typeof resultHandler !== "function" || typeof ops !== "object") if (typeof options === "function") return original.call(this, server, ns, ops, options);
					else return original.call(this, server, ns, ops, options, callback);
					const span = instrumentation.tracer.startSpan(`mongodb.${operationName}`, { kind: api_1.SpanKind.CLIENT });
		...
		_getV3PatchCommand() {
			...
			return (original) => {
				return function patchedServerCommand(server, ns, cmd, options, callback) {
					...
					const skipInstrumentation = instrumentation._checkSkipInstrumentation(currentSpan);
					const resultHandler = typeof options === "function" ? options : callback;
					if (skipInstrumentation || typeof resultHandler !== "function" || typeof cmd !== "object") if (typeof options === "function") return original.call(this, server, ns, cmd, options);
					...
					const span = instrumentation.tracer.startSpan(`mongodb.${type}`, { kind: api_1.SpanKind.CLIENT });
			...
			return (original) => {
				return function patchedV4ServerCommand(ns, cmd, options, callback) {
					...
					if (!skipInstrumentation) {
						span = instrumentation.tracer.startSpan(`mongodb.${commandType}`, { kind: api_1.SpanKind.CLIENT });
		...
		_getV3PatchFind() {
			...
			return (original) => {
				return function patchedServerCommand(server, ns, cmd, cursorState, options, callback) {
					...
					const skipInstrumentation = instrumentation._checkSkipInstrumentation(currentSpan);
					const resultHandler = typeof options === "function" ? options : callback;
					if (skipInstrumentation || typeof resultHandler !== "function" || typeof cmd !== "object") if (typeof options === "function") return original.call(this, server, ns, cmd, cursorState, options);
					else return original.call(this, server, ns, cmd, cursorState, options, callback);
					const span = instrumentation.tracer.startSpan("mongodb.find", { kind: api_1.SpanKind.CLIENT });
		...
		_getV3PatchCursor() {
			...
			return (original) => {
				return function patchedServerCommand(server, ns, cursorState, batchSize, options, callback) {
					...
					const skipInstrumentation = instrumentation._checkSkipInstrumentation(currentSpan);
					const resultHandler = typeof options === "function" ? options : callback;
					if (skipInstrumentation || typeof resultHandler !== "function") if (typeof options === "function") return original.call(this, server, ns, cursorState, batchSize, options);
					else return original.call(this, server, ns, cursorState, batchSize, options, callback);
					const span = instrumentation.tracer.startSpan("mongodb.getMore", { kind: api_1.SpanKind.CLIENT });
		...
		*/
		static _getCommandType(command) {
			if (command.createIndexes !== void 0) return internal_types_1.MongodbCommandType.CREATE_INDEXES;
			else if (command.findandmodify !== void 0) return internal_types_1.MongodbCommandType.FIND_AND_MODIFY;
			else if (command.ismaster !== void 0) return internal_types_1.MongodbCommandType.IS_MASTER;
			else if (command.count !== void 0) return internal_types_1.MongodbCommandType.COUNT;
			else if (command.aggregate !== void 0) return internal_types_1.MongodbCommandType.AGGREGATE;
			else return internal_types_1.MongodbCommandType.UNKNOWN;
		}
		/**
		...
		*/
		_populateV4Attributes(span, connectionCtx, ns, command, operation) {
			...
			let commandObj;
			if ((command === null || command === void 0 ? void 0 : command.documents) && command.documents[0]) commandObj = command.documents[0];
			else if (command === null || command === void 0 ? void 0 : command.cursors) commandObj = command.cursors;
		...
		*/
		...
		_addAllSpanAttributes(span, dbName, dbCollection, host, port, commandObj, operation) {
			...
			if (host && port) {
				...
				if (!isNaN(portNumber)) span.setAttribute(semantic_conventions_1.SEMATTRS_NET_PEER_PORT, portNumber);
			}
			...
			(0, instrumentation_1.safeExecuteInTheMiddle)(() => {
				...
				if (err) this._diag.error("Error running dbStatementSerializer hook", err);
			}, true);
		}
		...
		_scrubStatement(value) {
			if (Array.isArray(value)) return value.map((element) => this._scrubStatement(element));
			if (typeof value === "object" && value !== null) return Object.fromEntries(Object.entries(value).map(([key, element]) => [key, this._scrubStatement(element)]));
			return "?";
		}
		/**
		...
		*/
		_patchEnd(span, resultHandler, connectionId, commandType) {
			...
			return function patchedEnd(...args) {
				const error = args[0];
				if (span) {
					if (error instanceof Error) span === null || span === void 0 || span.setStatus({
				...
				return api_1.context.with(activeContext, () => {
					if (commandType === "endSessions") instrumentation._connectionsUsage.add(-1, {
						state: "idle",
						"pool.name": instrumentation._poolName
					});
		...
		setPoolName(options) {
			var _a, _b;
			const poolName = `mongodb://${(_a = options.hostAddress) === null || _a === void 0 ? void 0 : _a.host}:${(_b = options.hostAddress) === null || _b === void 0 ? void 0 : _b.port}/${options.dbName}`;
			this._poolName = poolName;
		}
		_checkSkipInstrumentation(currentSpan) {
			return this.getConfig().requireParentSpan === true && currentSpan === void 0;
		}
	};
}));
//#endregion
//#region node_modules/@opentelemetry/instrumentation-mongodb/build/src/types.js
...
//#endregion
//#region node_modules/@opentelemetry/instrumentation-mongodb/build/src/index.js
