...
import { a as InstrumentationNodeModuleDefinition, n as init_esm, s as InstrumentationBase } from "./@opentelemetry/instrumentation+[...].mjs";
//#region node_modules/@prisma/instrumentation/dist/index.mjs
...
function engineSpanKindToOtelSpanKind(engineSpanKind) {
	switch (engineSpanKind) {
		case "client": return import_src.SpanKind.CLIENT;
		default: return import_src.SpanKind.INTERNAL;
	}
}
var ActiveTracingHelper = class {
	...
	getTraceParent(context) {
		const span = import_src.trace.getSpanContext(context ?? import_src.context.active());
	...
	runInChildSpan(options, callback) {
		...
		if (options.middleware && !this.traceMiddleware) return callback();
		const tracer = this.tracerProvider.getTracer("prisma");
...
var package_default = {
	...
	dependencies: { "@opentelemetry/instrumentation": "^0.52.0 || ^0.53.0 || ^0.54.0 || ^0.55.0 || ^0.56.0 || ^0.57.0" },
...
var PrismaInstrumentation = class extends InstrumentationBase {
	...
	init() {
		return [new InstrumentationNodeModuleDefinition(MODULE_NAME, [VERSION])];
	}
	enable() {
		const config = this._config;
		const globalValue = { helper: new ActiveTracingHelper({
			traceMiddleware: config.middleware ?? false,
			tracerProvider: this.tracerProvider ?? import_src.trace.getTracerProvider(),
...
//#endregion
export { PrismaInstrumentation as t };
